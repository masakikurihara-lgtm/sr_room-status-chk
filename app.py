...
def display_room_status(profile_data, input_room_id):
    ...
    /* HTMLテーブルの基本スタイル (共通スタイルをできるだけ減らすため、ここは汎用記述のみ) */
    .stHtml .dataframe {
    ...
    /*
    🔥 🔝 参加イベント上位10ルームのテーブル専用CSS (既存の `.dataframe` をターゲット)
    */
    ...
    /*
    ✅ 📊 ルーム基本情報テーブル専用CSS (🔥 絶対共通使用を避けるための専用クラス 🔥)
    デグレを防ぐため、このテーブルにのみ適用されるように .dataframe-basic-room-info を使用
    */
    .stMarkdown table.dataframe-basic-room-info {
        border-collapse: collapse; /* 基本情報テーブルにも改めて指定 */
        margin-top: 10px; 
        width: 100%; 
        min-width: 800px; 
    }

    .stMarkdown table.dataframe-basic-room-info th,
    .stMarkdown table.dataframe-basic-room-info td {
        /* イベントテーブルのデザインを継承 */
        text-align: center !important; 
    ...
    .stMarkdown table.dataframe-basic-room-info tbody tr:hover {
        background-color: #f7f9fd; 
    }
    ...
    basic_info_html = df_basic_info.to_html(
        escape=False,
        index=False,
        classes='dataframe-basic-room-info data-table data-table-full-width' # ★専用のクラスを適用★
    )
